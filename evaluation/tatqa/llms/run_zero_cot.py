...
if __name__=="__main__":
        ...
        for index,row in enumerate(raw_data):
                if row.question.id() in ids and row.question.id() ==  raw_data[index+1].question.id():
                        ...
                        continue
                print(evidence_text)
                user_prompt = """Think step by step, use information from given table and text and answer the Question:"""+evidence_text +row.question.text()
                ...
                evidences = []
                final_questions.to_csv("chatgpt_tatqa_zero_cot.tsv",sep="\t",index=False)
