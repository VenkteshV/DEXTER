...
if __name__ == "__main__":
        ...
        for index, row in enumerate(raw_data):
                ...
                evidence_emb, evidences,5,0.5)
                evidence_text = " ".join(evidences)
                print("gt***",row.answer.flatten())
                user_prompt = """Given a question and relevant evidence use information from evidence Evidence:"""+evidence_text+""" and Think step by step, generate rationale preceded by rationale: and generate all possible and applicable preceded by [Final Answer]:. Start the set of answers with [Final Answer]: separated by newline for the Question:"""+row.question.text()
                ...
                final_questions.to_csv("chatgpt_ambigqa_oracle_zero_cot.tsv",sep="\t",index=False)
        print("F1", f1/len(ids),len(ids))
