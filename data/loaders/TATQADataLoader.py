    ...
    def load_raw_dataset(self, split=Split.TRAIN):
        ...
        for data in dataset:
            for question in data["questions"]:
                ...
                if(TABLE in question["answer_from"]):
                    self.raw_data.append(Sample(q.id()+"-"+str(count), q, question["answer"], TableEvidence(data["table"]["table"],idx=data["table"]["uid"])))
                    count+=1
                if(TEXT in question["answer_from"]):
