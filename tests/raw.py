...
def inference_span_predictor(tokenized_data,tokenizer,model, dev_data, save_predictions=False):
    # outputs = []
    # if True:
    ...
    #
    outputs = json.load(open("data/inf_outputs.json","r"))
    if save_predictions:
        ...
        n_paragraphs = None
    predictions = decode_span(outputs,tokenizer,tokenized_data,
                                       n_paragraphs=n_paragraphs)
...
inference_span_predictor(tokenized_data,loader.tokenizer.tokenizer,model=reader,dev_data=data_loader,save_predictions=False)
